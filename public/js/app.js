...
function filesApp() {
  ...
  topRow.appendChild(labelRow('Root', rootToggle));
  topRow.appendChild(labelRow('Hidden', hiddenToggle));
  ...
  renameTo.placeholder = 'new name';

  const moveTo = document.createElement('input');
  moveTo.placeholder = '/dest/dir';

  async function refreshList() {
    const data = await api('/api/files/list', { path: currentPath });
    const entries = (data.entries || []).filter(e => showHidden || !e.hidden);
    ...
    entries.forEach(e => {
      ...
      name.addEventListener('click', async () => {
        if (e.type === 'dir') {
          ...
          filePath.value = joinPath(currentPath, e.name);
          const res = await api('/api/files/read', { path: filePath.value });
      ...
      meta.textContent = `${e.type} ${e.perms || ''}`;
  ...
  const row = document.createElement('div');
  row.className = 'row';
  const btnList = btn('list', refreshList);
  ...
  const btnSearch = btn('search', async () => {
    const data = await api('/api/files/search', { path: currentPath, query: search.value });
    list.innerHTML = (data.results || []).map(r => `<div>${r}</div>`).join('');
  });
  ...
  const btnExtract = btn('extract', async () => {
    await api('/api/files/extract', { path: currentPath, name: 'archive_extracted' });
    notify('archive extracted');
    refreshList();
  });
  const btnDelete = btn('delete', async () => {
    await api('/api/files/delete', { path: filePath.value });
    notify('deleted');
    refreshList();
  });
  const btnRename = btn('rename', async () => {
    await api('/api/files/rename', { path: filePath.value, new_name: renameTo.value });
    notify('renamed');
    refreshList();
  });
  const btnMove = btn('move', async () => {
    await api('/api/files/move', { src: filePath.value, dest_dir: moveTo.value });
    notify('moved');
    refreshList();
  });
  ...
  wrap.appendChild(renameTo);
  wrap.appendChild(moveTo);
